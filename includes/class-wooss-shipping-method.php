		...
		class Wooss_Shipping_Method extends WC_Shipping_Method {
			...
			public function calculate_shipping( $package = array() ) {
				if ( is_cart() || is_checkout() ) {
					...
					$origin_country = $sendbox_data['wooss_country'];
					...
					//remove comma
					if (strpos($destination_state, ',') !== false) {
						$stateParts = explode(',', $destination_state);
						$destination_state = trim(end($stateParts));
					  }
					if ( empty( $destination_city ) ) {
						$destination_city = $package['destination']['city'];
					}

					if (strpos($destination_city, ',') !== false) {
						$cityParts = explode(',', $destination_city);
						$destination_city = trim($cityParts[0]);
					  }
					...
					$payload_array_data = array(
						...
						'currency'              => $currency,
						//updating based on the sendbox new change to pass service code September 25th 2024
				...
				//updating based on the sendbox new change to pass service code response change now returns rate.fee September 25th 2024

				$wooss_rates_type = $sendbox_data['wooss_rates_type'];
				if ( 'maximum' == $wooss_rates_type && isset( $delivery_quotes_details->rate) ) {
					$quotes_fee = $delivery_quotes_details->rate->fee;
				} elseif ( 'minimum' == $wooss_rates_type && isset( $delivery_quotes_details->rate ) ) {
					$quotes_fee = $delivery_quotes_details->rate->fee;
				}
					...
					$payload_array_data['currency'] = $currency;
					...
					//updating based on the sendbox new change to pass service code response now returns rate.fee September 25th 2024
...
function wooss_form_fields() {
	...
	if ( isset( $_GET['tab'] ) && ( $_GET['tab'] == 'shipping' && isset( $_GET['section'] ) && $_GET['section'] == 'wooss' && $shipping_methods_enabled == 'yes' )  ) {
		...
		if ( 200 == $profile_data_response_code ) {
			...
			$wooss_con = $profile_data_response_body->country->name;
				...
				<table style="width:100%">
					...
					<tr>
						...
						<td>
							...
							foreach ( $states_body  as $state_key => $states_values) {
								if(is_array($states_values)){
									sort($states_values);
									foreach($states_values as $ship_state_key => $ship_state_value){
										echo "<option value='$ship_state_value->name'>$ship_state_value->name</option>";
									}
	...
	foreach ( $payload_array_data as $key => $value ) {
		switch ( $key ) {
			...
			case 'origin_city':
				...
				case 'currency':
					$payload_data->currency = $value;
					break; 

		    case 'service_code':
				$payload_data->service_code = $value;
