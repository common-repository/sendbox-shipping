(function ($) {
    ...
    $(document).ready(function () {
        $('select#wooss_destination_country').select2();
        $('select#wooss_destination_state').select2();

        //start of new kini i am trying for origin
         $('select#wooss_origin_country').select2();
        $('select#wooss_origin_states_selected').select2(); 
        ...
        request_shipment_btn.on("submit click", function (e) {
            ...
            var wooss_origin_name = $("input#origin_name").val();
            $('input#origin_name').on('input',function(e){
              wooss_origin_name = $(this).val();
              });

              var wooss_origin_phone = $("input#origin_phone").val();
              $('input#origin_phone').on('input',function(e){
                wooss_origin_phone = $(this).val();
                });
           ...
           /*  var wooss_destination_street = $(
            ...
            //start of new origin kini i am trying
             var wooss_origin_country = $("select#wooss_origin_country").val();
            $("select#wooss_origin_country").change(function () {
                wooss_origin_country = $(this).val();
            }) 

            var wooss_origin_state = $("select#wooss_origin_states_selected").val();
            $("select#wooss_origin_states_selected").change(function () {
                wooss_origin_state = $(this).val();

            })

             var wooss_origin_street = $("input#origin_street").val();
             $('input#origin_street').on('input',function(e){
               wooss_origin_street = $(this).val();
               });

               var wooss_origin_city = $("input#origin_city").val();
               $('input#origin_city').on('input',function(e){
                 wooss_origin_city = $(this).val();
                 });

                 var wooss_destination_city = $("input#destination_city").val();
                 $('input#destination_city').on('input',function(e){
                   wooss_destination_city = $(this).val();
                   });

                   var wooss_destination_street = $("input#destination_street").val();
                   $('input#destination_street').on('input',function(e){
                     wooss_destination_street = $(this).val();
                     });
        ...
        $("select#wooss_destination_country").on('change ', function () {
            ...
            wooss_selected_country_code = wooss_selected_option.data("countryCode");
            $.blockUI();

            $.post(wooss_ajax_object.wooss_ajax_url, {
        ...
        //doing for origin state
         $("select#wooss_origin_country").on('change ', function () {
            var wooss_selected_option = jQuery(this).children("option:selected");
            wooss_selected_country = wooss_selected_option.val();
            wooss_selected_country_code = wooss_selected_option.data("countryCode");
            $.blockUI();
            ...
            $.post(wooss_ajax_object.wooss_ajax_url, {
                ...
                function (response) {
                    if (response) {
                        $("select#wooss_origin_states_selected option:gt(0)").remove();
                        $('select#wooss_origin_states_selected').append(response);
                        $.unblockUI();
                    }
                });
